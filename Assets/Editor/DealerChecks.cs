...
public static class DealerChecks
{
    ...
    public static string Run()
    {
        ...
        var g=new TableRun(rules);g.Reset(123);
        int money=g.coins;
        ...
        Action<OfferKind,int> one=(kind,target)=>{g.Reset(123);g.coins=0;g.offers.Clear();g.offers.Add(new DealerOffer(kind,target){revealed=true});};
        ...
        g.EnterWorkshop();check(g.bonusHands==1&&g.HandsLeft==9,"Bonus hand persists during stage");
        g.phase=RunPhase.StageClear;g.Advance();check(g.bonusHands==0&&g.HandsLeft==8,"Bonus resets next stage");
        g.RevealOffer(0);g.LeaveWorkshop();check(g.offers.Count==0&&!g.RevealOffer(0)&&!g.ApplyOffer(0),"Leaving expires rewards and blocks dealer actions");
        ...
        g.Reset(12);for(int i=5;i<12;i++){g.coins=100;check(g.RevealOffer(i),"All12 picks selectable");}
